...
class TrackTestRunnerNode(TestRunnerBase):
    """TrackTestRunnerNode is a ROS2 node that manages the orchestration of track tests."""
    ...
    def __init__(self):
        ...
        self.declare_common_test_runner_parameters(
            {'test_timeout': 150.0, 'test_cooldown_time': 0.0}
        )
        ...
        self.topics_to_record = list(self.default_topics_to_record)
        ...
        self.configure_test_config_index_bounds(len(self.test_configurations))
    ...
    def setup_status_callback(self, msg: SetupState):
        super().setup_status_callback(msg)
        self.setup_status_subscriber_callback(msg)
    ...
    def parse_test_configs(self):
        ...
        track_test_configurations = self.load_json_list(
            self.test_configurations_json_path,
            ('track_test_configurations',),
        ...
        try:
            for track_test_config in track_test_configurations:
                ...
                if 'test_start_point' in test_profile and test_profile['test_start_point'] != '':
                    start_point_value = test_profile['test_start_point']
                    x, y, z, roll, pitch, yaw = map(float, start_point_value.split(','))
                    ...
                    test_start_point.pose.orientation.x = quaternion[0]
                    test_start_point.pose.orientation.y = quaternion[1]
                    test_start_point.pose.orientation.z = quaternion[2]
                    test_start_point.pose.orientation.w = quaternion[3]

                topics_to_record = test_profile.get('topics_to_record', self.topics_to_record)
                parsed_config = {
                    'iterations': iterations,
                    'preplanned_route_filepath': test_profile.get(
                        'preplanned_route_filepath', self.preplanned_route_filepath
                    ),
                    'vehicle_start_position_is_test_start_point': test_profile.get(
            ...
            self.get_logger().error(f'Failed to parse track test configuration: {exc}')

        return parsed_configs

    def configure_test_config_index_bounds(self, config_count: int):
        ...
        if requested_end_index <= 0:
            ...
            self.get_logger().warn(
                ...
                f'track test configuration count {config_count}; using {config_count}.'
            )
            self.test_config_end_index = config_count
    ...
    def start_rosbag_recording(self, topics_to_record):
        """Start a rosbag using the common runner process lifecycle."""
        super().start_rosbag_recording(
            topics_to_record,
            str(int(time.time())),
            process_label='track_test_rosbag',
        )
    ...
    def initialize_next_test(self):
        ...
        self.current_iter_idx += 1
        if (
            self.current_config_index >= len(self.test_configurations)
            or self.current_config_index >= self.test_config_end_index
        ):
...
def main(args=None):
    ...
    track_test_runner_node = TrackTestRunnerNode()
