...
class DFRobot_CH432T(SerialBase, object):
    ...
    def __init__(
        self, port="CH432T_PORT_1", baudrate=115200, bytesize=8, parity="N", stopbits=1
    ):
        """!
        @brief Module init
        ...
        if port not in ["CH432T_PORT_1", "CH432T_PORT_2"]:
            raise SerialException(
                "Invalid PORT, please select 'CH432T_PORT_1' or 'CH432T_PORT_2'"
            )
        ...
        # self._spi.mode = 0  # Set SPI mode to 0 (CPOL=0, CPHA=0)
        # self._spi.no_cs = True
        self._spi.max_speed_hz = 1_000_000  # SPI communication frequency is default to be 1 MHz
    ...
    def close(self):
        ...
        self.set_low_power_mode(
            CH432T_LOW_POWER_MODE
        )  # close the internal reference clock of the serial port
        self.is_open = False
    ...
    def open(self):
        ...
        if 0x66 != scr:
            raise SerialException(
                "Failed to open port! Check whether the expansion board \
    ...
    def _reconfigure_port(self, force_update=False):
        ...
        if force_update or [cflag, self._baudrate] != self.orig_attr:
            ...
            self._write_reg(
                CH432T_MCR_REG, CH432T_MCR_RTS_BIT | CH432T_MCR_OUT2 | CH432T_MCR_AFE
            )
    ...
    def set_baudrate(self, baud):
        ...
        mode = int(
            clock_rate / 16 / baud
        )  # The set value corresponding to the baud rate in the current mode
    ...
    def _write_reg(self, reg, data):
        ...
        with ch432t_spi_lock:
            ...
            logger.info(
                "portnum = %d, reg = %d, reg_addr = %#x, data = %#x"
                % (self.portnum, reg, reg_addr[0], data[0])
            )
    ...
    def _read_reg(self, reg, length):
        ...
        with ch432t_spi_lock:
            ...
            logger.info(
                "portnum = %d, reg = %d, reg_addr = %#x, rslt = %#x"
                % (self.portnum, reg, temp, reg_addr[1])
            )
            reg_addr.pop(0)
            for i in range(0, 5):
                for j in range(0, 30):
    ...
    def enable_parity_bit(self, mode):
        ...
        """
        if mode:
            self._reg_bit_update(
                CH432T_LCR_REG, CH432T_LCR_PARITY_EN_BIT, CH432T_LCR_PARITY_EN_BIT
            )
        else:
