...
def safe_modbus_write(master, slave, register, write_value):
    try:
        return master.execute(
            slave, cst.WRITE_SINGLE_REGISTER, register, output_value=write_value
        )
    except Exception as e:
...
def change_slave_id(master, new_slave_id):
    ...
    try:
        ...
        logging.info("\n--- IMPORTANT ---")
        logging.info(f"The sensor's address *should* now be set to {new_slave_id} ({hex(new_slave_id)}).")
