...
public:
  ...
  : Node("teleop_gateway")
  {
    ...
    reverse_steer_correction_topics_ = this->declare_parameter(
      "reverse_steer_correction_topics", std::vector<std::string>({"joy_vel"}));
    ...
    RCLCPP_INFO(
      get_logger(),
      "emergency_stop can be ACTIVATED using button %d and CLEARED using button %d of joystick. It can also be managed by publishing boolean data on /emergency_stop topic.",
      emergency_stop_set_joy_button_index_,
      emergency_stop_clear_joy_button_index_);
    RCLCPP_INFO(
      get_logger(),
      "To activate emergency_stop from command line: ros2 topic pub --once /emergency_stop std_msgs/msg/Bool \"data: true\"");
    RCLCPP_INFO(
      get_logger(),
      "To clear emergency_stop from command line: ros2 topic pub --once /emergency_stop std_msgs/msg/Bool \"data: false\"");
    ...
    for (size_t i = 0; i < reverse_steer_correction_topics_.size(); ++i) {
      ...
      reverse_steer_correction_subscribers_[i] =
      ...
      reverse_steer_correction_publishers_[topic_name] =
    ...
    emergency_stop_subscriber_ = this->create_subscription<std_msgs::msg::Bool>(
      "/emergency_stop", 10,
      std::bind(&TeleopGateway::emergency_stop_callback, this, std::placeholders::_1));
    ...
    joy_watchdog_timer_ = this->create_wall_timer(
      ...
      std::bind(&TeleopGateway::joy_watchdog_callback, this));
  }

private:
  void reverse_steer_correction_subscriber_callback(
    const geometry_msgs::msg::Twist::SharedPtr twist_msg, const std::string & topic_name)
  {
  ...
  void mux_callback(const geometry_msgs::msg::Twist::SharedPtr twist_msg)
  {
    if (emergency_stop_value_) {
      ...
      twist_msg->linear.x =
        (std::abs(twist_msg->linear.x) < min_allowed_linear_velocity_) ? 0.0 : twist_msg->linear.x;
      twist_msg->angular.z =
        (std::abs(twist_msg->angular.z) <
        min_allowed_angular_velocity_) ? 0.0 : twist_msg->angular.z;
  ...
  void joy_watchdog_callback()
  {
    ...
    if (time_since_last_msg > joy_emergency_stop_timeout_) {
      if (is_joy_alive_) {
        ...
        RCLCPP_WARN(
          get_logger(),
          "/joy topic has stopped publishing for %.2f seconds. emergency_stop ACTIVATED.",
          time_since_last_msg);
        is_joy_alive_ = false;
      }
    } else {
      if (!is_joy_alive_) {
        ...
        if (emergency_stop_value_) {
          RCLCPP_WARN(
            get_logger(),
            "Receiving messages from /joy topic now, but emergency_stop is ACTIVATED.");
        } else {
          RCLCPP_WARN(
            get_logger(),
  ...
  reverse_steer_correction_subscribers_;
  std::map<std::string, rclcpp::Publisher<geometry_msgs::msg::Twist>::SharedPtr>
  reverse_steer_correction_publishers_;
